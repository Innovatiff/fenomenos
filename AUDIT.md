...
| G4 | `js/app.js` initMap (`renderWorldCopies:false`, bounds ±179.9, −80/84) | Antimeridiano y polos | **RESUELTO en Fase 2**: proyección globo por defecto (MapLibre v5, `projectionApply()`) — el mundo gira completo sin costuras ni límites; el modo plano (a elección) conserva la contención del bug §1. Verificado con el MapLibre real: alternar globo/plano y saltar al antimeridiano sin errores |
